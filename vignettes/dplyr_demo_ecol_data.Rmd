...
summary_vars <- left_join(mean_percent_cover, diversity_vars) %>%
        semi_join(., {analysis_data %>% 
                        select(transect_number,management,years_since)})
```

For the difference between a `semi_join` and a `left_join`, see the section on relational data frames in Wickham and Grolemund's book [R for datascience](http://r4ds.had.co.nz/relational-data.html).
...
That's nice. But I wonder how each of the variables relate to one another? 
